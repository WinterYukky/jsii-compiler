...
|---|---|---|---|---|
| 0 | **#4700** (already open) | `checker.getFullyQualifiedName` | — | jsii's `symbolId` needs it. Nudge/rebase if stale. |
...
| 2 | **`getEmitOutput` API** | the draft emit API from `draft/api-emit-1784711177` + writeToDisk + osvfs fix (21fd26fb, 6571a5e9) folded in from the start | none (independent of 1) | Propose emit API **with** the write-to-disk option in one PR — the 345MB-single-response shape should never ship as the only mode; our data is the motivation. |
| 3 | **Batched `getSymbolDocumentations`** | 332e9bbc rebased onto main | none (independent of 1 & 2) | Additive endpoint + client method. Motivated by the −30%-requests measurement. |
...
3. Submit PR 1 (cache) → PR 2 (emit) → PR 3 (batch docs), referencing the issue.
