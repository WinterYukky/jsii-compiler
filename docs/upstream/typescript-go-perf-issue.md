...
We ported [jsii](https://github.com/aws/jsii-compiler) (the AWS CDK
multi-language binding generator) onto the `@typescript/native-preview` API.
jsii walks every exported symbol, type and doc comment of a package, so it
exercises the API's out-of-process design about as hard as any tool can.

Headline results on `aws-cdk-lib` (20,744 types / ~100k members):

- **The port works, and is 3.4x faster end-to-end** than the strada-based
  pipeline (119s → 35.5s), with output parity enforced at every step.
...
Below: the measurements that isolate that conclusion, four API suggestions,
and three parity-proven patches we're happy to PR.
...
`getEmitOutput`. All numbers are 3-run medians on the same instance; every
...
2. **Huge single responses hurt too.** A full-project `getEmitOutput` on
   aws-cdk-lib returned ~345 MB in one response (63% of all received bytes).
   An optional `writeToDisk` mode — the server writes outputs through the
   same code path the CLI uses, and the response carries names only — removed
   the transfer entirely, with byte-identical outputs on disk.
...
3. **A write-to-disk (or streaming) mode for `getEmitOutput`** instead of a
   single huge response.
4. **Input-keyed caching on Checker query methods** (node /
   symbol+location), valid per snapshot.

## Patches (parity-proven, ready to PR)

- Suggestion 4: `WinterYukky/typescript-go@feat/checker-input-cache`
- Suggestion 3: `WinterYukky/typescript-go@feat/api-get-emit-output`
